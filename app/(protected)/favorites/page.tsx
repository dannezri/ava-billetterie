...
export default function FavoritesPage() {
  ...
  const removeMutation = useMutation({
    ...
    onError: () => {
      toast({ title: 'Erreur', description: 'Impossible de retirer ce favori.', variant: 'destructive' });
    },
  });
          ...
          {data?.favorites.map((favorite) => (
            <Card key={favorite.id} className="overflow-hidden">
              ...
              <CardContent className="p-4">
                <h3 className="font-semibold text-lg mb-1">{favorite.event.title}</h3>
                {favorite.event.artist && (
                  <p className="text-sm text-muted-foreground mb-2">{favorite.event.artist}</p>
                )}
                <div className="space-y-1.5 text-sm text-muted-foreground">
                  ...
                  {favorite.event.ticketsAvailable > 0 ? (
                    <div className="flex items-center gap-2">
                      <Ticket className="h-4 w-4 shrink-0" />
                      {favorite.event.ticketsAvailable} billet
                      {favorite.event.ticketsAvailable > 1 ? 's' : ''} dès{' '}
                      {favorite.event.minPrice.toFixed(2)} €
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      <Ticket className="h-4 w-4 shrink-0" />
