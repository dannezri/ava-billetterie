/**
 * Page Succès - Achat Groupe Confirmé
 * Route: /checkout/group/success?transactions=id1,id2&event=eventId
 */
...
export default function GroupSuccessPage() {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        ...
        const ev = results[0]?.data?.transaction?.ticket?.event;
        if (ev) {
          setEvent({
            title: ev.title,
  ...
  const totalTicketsPrice = totalPaid - totalFees;
    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/20 py-16">
      <div className="container mx-auto max-w-3xl px-4">
        ...
        <div className="mb-10 text-center">
          <div className="mx-auto mb-5 flex h-24 w-24 items-center justify-center rounded-full bg-green-100">
            <div className="relative">
              ...
              <Users className="absolute -bottom-1 -right-1 h-6 w-6 rounded-full bg-green-100 text-green-600" />
            </div>
          </div>
          ...
          <Badge className="mt-3 gap-2 bg-green-600 text-white px-4 py-1.5 text-sm">
            <Users className="h-4 w-4" />
            {transactions.length} places côte à côte confirmées
          </Badge>
        </div>
            ...
            <CardContent className="p-6 space-y-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold">
                            {tx.ticket.section ?? 'Placement libre'}
                            {tx.ticket.row && ` · Rang ${tx.ticket.row}`}
                          </p>
                          {tx.ticket.seatNumber && (
                            <p className="text-xs text-muted-foreground flex items-center gap-1">
                              <Ticket className="h-3 w-3" />
                              Siège {tx.ticket.seatNumber}
                            </p>
                          )}
                        </div>
                      </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span>{totalTicketsPrice.toFixed(2)}€</span>
                </div>
            ...
            <div className="flex items-start gap-3 rounded-lg border p-4">
              ...
              <div>
                ...
                <p className="mb-3 text-sm text-muted-foreground">
                  Vos {transactions.length} billets sont disponibles dans votre compte
                </p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-green-800">
                  Votre achat est couvert. En cas de billet invalide ou d'événement annulé,
        ...
        <p className="mt-8 text-center text-xs text-muted-foreground">
          Références : {transactionIds.map((id) => id.slice(0, 8)).join(' · ')}
        </p>
      </div>
    </div>
  );
