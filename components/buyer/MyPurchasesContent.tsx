      ...
      <Card className="hover:bg-accent transition-colors cursor-pointer">
        <CardContent className="p-6">
          <div className="flex items-start gap-4">
            ...
            {purchase.ticket.event.image_url && (
              <img
                src={purchase.ticket.event.image_url}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-4 mt-3 text-sm text-muted-foreground">
                <div className="flex items-center gap-1">
                  ...
                  {format(new Date(purchase.ticket.event.event_date), 'PPP', { locale: fr })}
                </div>
                <div className="flex items-center gap-1">
              ...
              <div className="flex items-center gap-4 mt-4">
                <div className="text-sm">
                  ...
                  <span className="font-medium">{purchase.ticket.seat_category}</span>
                </div>
                <div className="text-sm">
              ...
              <div className="flex items-center gap-2 mt-4">
                {purchase.status === 'ESCROWED' && purchase.escrow_release_date && (
                  <Badge variant="outline" className="flex items-center gap-1">
                    <Clock className="h-3 w-3" />
                    Séquestre jusqu'au{' '}
                    {format(new Date(purchase.escrow_release_date), 'dd/MM/yyyy')}
                  </Badge>
                )}
                {purchase.dispute && (
...
function StatusBadge({ status }: { status: string }) {
  const variants: Record<string, { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline' }> = {
    ...
    ESCROWED: { label: 'En séquestre', variant: 'default' },
